...
function CronTab(u, f, cb) {
  ...
  this.save = function (cb) {
    if (file) {
      ...
      var command = makeChildCommand("save");
  ...
  function saveFile(crontab, cb) {
    fs.writeFile(file, crontab.render(), function (err) {
      if (err) cb && cb({ message: stderr }, self);
      else cb && cb(null, self);
    });
  }
  /**
    ...
    lines.forEach(function (line) {
      if (typeof line === "string") {
        ...
        if (trimmed === pausedFlag) {
          pausedTokens.push(line.slice(pausedFlag.length));
        }
      }
    });
  ...
  function load(cb) {
    if (file) {
      fs.readFile(file, (err, data) => {
        if (err) parseData(err.code, null, err.message, cb);
        else parseData(0, data ? data.toString() : "", null, cb);
      });
    } else {
      var stdout = "";
      var stderr = "";
      var args = makeChildArgs("load");
      var command = makeChildCommand("load");
      var child = Spawn(command, args);
      ...
      child.on("error", function (err) {});
      child.on("close", function (code) {
        parseData(code, stdout, stderr, cb);
      });
    }
  }

  /**
   * Parses data from command / file
   *
   * @param {Number} code
   * @param {Sgring} stdout
   * @param {String} stderr
   ...
   * @api private
   */
  function parseData(code, stdout, stderr, cb) {
   ...
   * @api private
   */
  function makeChildCommand(action) {
    var command;
    if (file && action) {
      command = "cat";
...
module.exports = {
  load: function () {
    if (_.isString(arguments[0]) && _.isFunction(arguments[1])) {
      ...
      _.isString(arguments[1]) &&
      _.isFunction(arguments[2])
    ) {
      new CronTab(arguments[0], arguments[1], arguments[2]);
    }
  },
};
